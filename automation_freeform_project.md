...
|------|------|
| 선정 도구 | [도구명] |
| 선정 이유 | ① 무료 범위 : Wait 노드로 장시간 대기를 걸어도 오퍼레이션 소모 개념이 없어, 7일 후 재시험 안내 예약 발송 구조를 무료 범위 안에서 안정적으로 운영 가능 ② 데이터 처리 유연성 : Google Forms에서 0점이 빈 값으로 들어오는 비정형 데이터를 Code 노드의 JavaScript로 즉시 보정할 수 있어 예외 처리에 유리 |
